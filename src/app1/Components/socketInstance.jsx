...
// export  const socketURL = `${protocol}//${host}:${port}`; // If no port, use `${protocol}//${host}`
export const socketURL = `${protocol}//${host}`;
...
export const getSocket = (forceNew = false) => {
  if (forceNew || !socket) {
    if (socket) {
      socket.disconnect();  // Cleanup previous connection if exists
    }
    socket = io(socketURL, socketConfig);
  }
  return socket;
