...
import { setDocxFile } from "../features/ReportSlice";
...
const DocxViewer = ({ fileId }) => {
    ...
    // Function to store DOCX file in Redux
    const storeDocxInRedux = (docxBlob, fileId) => {
        const docxFile = {
            id: fileId,
            blob: docxBlob,
        ...
        dispatch(setDocxFile(docxFile)); // You can rename this action to setDocumentFile
        console.log("DOCX stored in Redux for file ID:", fileId);
    };
    ...
    useEffect(() => {
        const fetchDocx = async () => {
            ...
            try {
                const response = await fetch(`${socketURL}/get-report/${selectedFileId}`);
                ...
                // Store DOCX file in Redux
                storeDocxInRedux(blob, selectedFileId);
            } catch (err) {
    ...
    return (
        ...
        >
            ...
            <Card style={{ marginLeft: "16px",  overflowY: "auto", width:'100%' }}>
                <div style={{
                    position: "sticky",
                    top: "0",
                    background: "#fff",
